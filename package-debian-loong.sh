...
write_launcher_file() {
  ...
  install -m 755 /dev/stdin "$stage/usr/bin/firefly" <<'EOF'
#!/usr/bin/env bash
set -euo pipefail
DIR="/opt/v2rayN"
cd "$DIR"

if [[ -x "$DIR/Firefly" ]]; then
  exec "$DIR/Firefly" "$@"
fi
...
write_desktop_file() {
  ...
  install -m 644 /dev/stdin "$stage/usr/share/applications/firefly.desktop" <<'EOF'
[Desktop Entry]
Type=Application
Name=Firefly
Comment=Firefly for Debian GNU Linux
Exec=firefly
Icon=firefly
Terminal=false
Categories=Network;
...
package_binary() {
  ...
  [[ -f "$icon_candidate" ]] && cp "$icon_candidate" "$stage/usr/share/icons/hicolor/256x256/apps/firefly.png" || true
...
EOF
  ...
  [[ -f "$stage/opt/v2rayN/Firefly" ]] && chmod 0755 "$stage/opt/v2rayN/Firefly" || true

  deb_out="$OUTPUT_DIR/Firefly_${VERSION}_${deb_arch}.deb"
