...
function Assert-FireflyClientApiUrl {
    $endpoint = $null
    if ([string]::IsNullOrWhiteSpace($env:FIREFLY_CLIENT_API_URL) -or
        ...
        throw 'FIREFLY_CLIENT_API_URL must be a non-empty absolute HTTPS URL. Set it from a protected environment variable before packaging.'
    }
}

Assert-FireflyClientApiUrl

$repoRoot = Split-Path -Parent $PSCommandPath
$project = Join-Path $repoRoot 'v2rayN\v2rayN.Desktop\v2rayN.Desktop.csproj'
...
$versionFile = Join-Path $repoRoot 'v2rayN\Directory.Build.props'
...
function Invoke-WindowsPackage {
    ...
    $assetsFile = Join-Path $repoRoot 'v2rayN\v2rayN.Desktop\obj\project.assets.json'
    ...
    $upstreamExe = Join-Path $publishDirectory 'v2rayN.exe'
    ...
    if ($PackageFormat -ne 'installer') {
        ...
        if ($archiveEntries -contains 'v2rayN.exe') {
            throw "Package verification failed for ${rid}: root v2rayN.exe is present."
        }
    ...
    Write-Host "Verified ${rid}: $productName.exe; no root v2rayN.exe; sing-box, Xray, and Mihomo cores included."
}
