...
version="$(sed -nE 's/.*<Version>([^<]+)<\/Version>.*/\1/p' "$repo_root/v2rayN/Directory.Build.props" | head -n 1)"
...
[[ -f "$publish_directory/v2rayN" ]] || { echo "Published macOS executable is missing: $publish_directory/v2rayN" >&2; exit 1; }
...
chmod +x "$macos_path/v2rayN" "$macos_path/bin/sing_box/sing-box" "$macos_path/bin/xray/xray" "$macos_path/bin/mihomo/mihomo"
