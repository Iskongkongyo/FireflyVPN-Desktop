...
PROJECT_HINT="Firefly.Desktop/Firefly.Desktop.csproj"
...
prepare_workspace() {
  ...
  [[ -f "$PROJECT" ]] || PROJECT="$(find . -maxdepth 3 -name 'Firefly.Desktop.csproj' | head -n1 || true)"
  [[ -f "$PROJECT" ]] || die "Firefly.Desktop.csproj not found"
}
...
populate_assets_zip_mode() {
  ...
  tmp="$(mktemp -d)"
  curl -fL "$url" -o "$tmp/Firefly-upstream.zip" || { echo "[!] Bundle download failed"; rm -rf "$tmp"; return 1; }
  unzip -q "$tmp/Firefly-upstream.zip" -d "$tmp" || { echo "[!] Bundle unzip failed"; rm -rf "$tmp"; return 1; }
...
[ -f %{buildroot}/opt/v2rayN/Firefly ] && chmod 0755 %{buildroot}/opt/v2rayN/Firefly || :

install -dm0755 %{buildroot}%{_bindir}
install -m0755 /dev/stdin %{buildroot}%{_bindir}/firefly << 'EOF'
#!/usr/bin/bash
set -euo pipefail
DIR="/opt/v2rayN"

if [[ -x "$DIR/Firefly" ]]; then exec "$DIR/Firefly" "$@"; fi

for dll in Firefly.Desktop.dll Firefly.dll; do
...
install -m0644 /dev/stdin %{buildroot}%{_datadir}/applications/firefly.desktop << 'EOF'
[Desktop Entry]
Type=Application
Name=Firefly
Comment=Firefly for Red Hat Enterprise Linux
Exec=firefly
Icon=firefly
Terminal=false
Categories=Network;
...
install -m0644 %{_builddir}/__PKGROOT__/firefly.png %{buildroot}%{_datadir}/icons/hicolor/256x256/apps/firefly.png
...
%{_bindir}/firefly
/opt/v2rayN
%{_datadir}/applications/firefly.desktop
%{_datadir}/icons/hicolor/256x256/apps/firefly.png
SPEC
...
package_binary() {
  ...
  cp "$icon_candidate" "$workdir/$PKGROOT/firefly.png"
  ...
  for f in "${RPM_TOPDIR}/RPMS/${archdir}/v2rayN-${VERSION}-1"*.rpm; do
    [[ -e "$f" ]] || continue
    firefly_rpm="$(dirname "$f")/$(basename "$f" | sed 's/^v2rayN-/Firefly-/')"
    mv "$f" "$firefly_rpm"
    echo "  $firefly_rpm"
    BUILT_RPMS+=("$firefly_rpm")
  done
}
