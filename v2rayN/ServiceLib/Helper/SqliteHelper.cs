...
public sealed class SQLiteHelper
{
    ...
    {
        return await _dbAsync.InsertAllAsync(await PrepareModelsAsync(models), runInTransaction: true).ConfigureAwait(false);
    }

    public async Task<int> InsertAsync(object model)
    {
        return await _dbAsync.InsertAsync(await PrepareModelAsync(model));
    }

    public async Task<int> ReplaceAsync(object model)
    {
        return await _dbAsync.InsertOrReplaceAsync(await PrepareModelAsync(model));
    }

    public async Task<int> UpdateAsync(object model)
    {
        return await _dbAsync.UpdateAsync(await PrepareModelAsync(model));
    }

    public async Task<int> UpdateAllAsync(IEnumerable models)
    {
        return await _dbAsync.UpdateAllAsync(await PrepareModelsAsync(models), runInTransaction: true).ConfigureAwait(false);
    }

    public async Task<int> DeleteAsync(object model)
    {
        if (model is ProfileItem profile)
        {
            await _dbAsync.DeleteAsync(new FireflyManagedProfileSecret { IndexId = profile.IndexId });
        }
    ...
    private static async Task<object> PrepareModelAsync(object model)
    {
        return model is ProfileItem profile
            ? await FireflyManagedProfileStorage.PrepareForStorageAsync(profile)
            : model;
    }

    private static async Task<List<object>> PrepareModelsAsync(IEnumerable models)
    {
        var result = new List<object>();
        foreach (var model in models)
        {
            result.Add(await PrepareModelAsync(model));
        }
        return result;
    }
}
