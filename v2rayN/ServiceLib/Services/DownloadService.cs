...
public sealed record DownloadStringResult(string Content, string? ClientIp);
...
public class DownloadService
{
    ...
    {
        var result = await TryDownloadStringWithHeaders(url, blProxy, userAgent);
        return result?.Content;
    }

    /// <summary>
    /// Tries to download string content and preserves the Firefly Worker client IP response header.
    /// </summary>
    ...
    /// <summary>
    /// Tries to download string content with a specified proxy and preserves response metadata.
    /// </summary>
    public async Task<DownloadStringResult?> TryDownloadStringWithHeaders(string url, IWebProxy? webProxy, string userAgent)
    {
        ...
        try
        {
            ...
            {
                return new DownloadStringResult(result2, null);
            }
        }
        catch (Exception ex)
        {
    ...
    {
        var result = await TryDownloadStringWithHeaders(url, webProxy, userAgent);
        return result?.Content;
    }
    ...
    private async Task<DownloadStringResult?> DownloadStringWithHeadersAsync(string url, IWebProxy? webProxy, string userAgent, int timeout)
    {
        try
        {
            ...
            var clientIp = response.Headers.TryGetValues(FireflySubscriptionCodec.ClientIpHeaderName, out var values)
                ? values.FirstOrDefault()
                : null;

            return new DownloadStringResult(content, clientIp);
        }
        catch (Exception ex)
        {
            Logging.SaveLog(_tag, ex);
