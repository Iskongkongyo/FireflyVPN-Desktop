...
public class ProcessService : IDisposable
{
        ...
        string? cleanupFilePath = null)
    {
        ...
        _cleanupFilePath = cleanupFilePath;
        ...
        _process.Exited += (_, _) => CleanupGeneratedConfig();
    }
    ...
    /// <summary>
    /// Used during a core reload: the next core has already written a new
    /// configuration at the same path, so the old process must not delete it.
    /// </summary>
    public void SuppressConfigCleanup()
    {
    ...
    {
        if (_cleanupFilePath.IsNotEmpty())
        {
            FireflyRuntimeArtifactPolicy.CleanupGeneratedConfig(_cleanupFilePath);
        }
    }
}
