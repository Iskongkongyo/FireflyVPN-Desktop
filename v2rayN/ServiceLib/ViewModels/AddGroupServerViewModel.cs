...
public partial class AddGroupServerViewModel : MyReactiveObject, ICloseable
{
    ...
    public async Task UpdatePreviewList()
    {
        ...
        var previewItems = await GroupProfileManager.GetChildProfileItemsByProtocolExtra(protocolExtra);
        AllProfilePreviewItemsObs.Clear();
        if (await FireflyManagedSubscriptionPolicy.ContainsManagedProfilesAsync(previewItems))
        {
            NoticeManager.Instance.Enqueue(FireflyManagedSubscriptionPolicy.ProtectedMessage);
            return;
        }
        AllProfilePreviewItemsObs.AddRange(previewItems);
    }
    ...
    private async Task SaveServerAsync()
    {
        ...
        if (await FireflyManagedSubscriptionPolicy.ContainsManagedProfileAsync(SelectedSource))
        {
            NoticeManager.Instance.Enqueue(FireflyManagedSubscriptionPolicy.ProtectedMessage);
            return;
        }
