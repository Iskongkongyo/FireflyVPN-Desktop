...
public partial class CheckUpdateViewModel : MyReactiveObject
{
    ...
    public ReactiveCommand<RxVoid, RxVoid> CheckFireflyUpdateCmd { get; }
    ...
    public CheckUpdateViewModel()
    {
        ...
        CheckFireflyUpdateCmd = ReactiveCommand.CreateFromTask(CheckFireflyUpdate);
        CheckFireflyUpdateCmd.ThrownExceptions.Subscribe(ex =>
        {
            Logging.SaveLog(_tag, ex);
    ...
    private async Task CheckFireflyUpdate()
    {
        var update = await new FireflyWorkerService().GetDesktopUpdateAsync(_config, false);
        if (update is null)
        {
            NoticeManager.Instance.SendMessageAndEnqueue(ResUI.TbFireflyAlreadyLatest);
            return;
        }
    ...
    private async Task CheckOnlyTask()
    {
        ...
        for (var k = CheckUpdateModels.Count - 1; k >= 0; k--)
        {
            ...
            var result = await updateService.CheckHasUpdateOnly(item.CoreType.Value, false);
    ...
    private async Task UpgradeCore()
    {
        foreach (var item in _lstUpdated)
        {
            ...
            else
            {
                ...
                NormalizeWindowsX86Mihomo(toPath, item.CoreType);
            }
    ...
    private static void NormalizeWindowsX86Mihomo(string toPath, ECoreType? coreType)
    {
        if (!Utils.IsWindows() || RuntimeInformation.ProcessArchitecture != Architecture.X86 || coreType != ECoreType.mihomo)
        {
            return;
        }

        var downloadedCore = Directory.GetFiles(toPath, "mihomo-windows-386*.exe", SearchOption.TopDirectoryOnly)
                                      .FirstOrDefault();
        if (downloadedCore.IsNullOrEmpty())
        {
            return;
        }

        File.Move(downloadedCore, Path.Combine(toPath, "mihomo.exe"), true);
    }
