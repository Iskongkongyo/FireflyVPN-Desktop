...
public partial class MainWindowViewModel : MyReactiveObject
{
    ...
    private async Task Init()
    {
        ...
        var nodeFetchNotificationId = NoticeManager.Instance.BeginFireflyNodeFetch();
        try
        {
            ...
            if (fireflySync.Enabled)
            {
                foreach (var subId in fireflySync.SubscriptionIds)
                {
                    ...
                    await SubscriptionHandler.UpdateProcess(_config, subId, false, StartupSubscriptionUpdateHandler);
                }
        ...
        {
            NoticeManager.Instance.EndFireflyNodeFetch(nodeFetchNotificationId);
        }
