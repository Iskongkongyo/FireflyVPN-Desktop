...
public class CoreManager
{
    ...
    public async Task LoadCore(CoreConfigContext? mainContext, CoreConfigContext? preContext)
    {
        ...
        {
            FireflyRuntimeArtifactPolicy.CleanupGeneratedConfig(fileName);
            await UpdateFunc(true, result.Msg);
            return;
        }

        var nodeSummary = await FireflyManagedSubscriptionPolicy.GetSafeSummaryAsync(node);
        await UpdateFunc(false, nodeSummary);
        await UpdateFunc(false, $"{Utils.GetRuntimeInfo()}");
        await UpdateFunc(false, string.Format(ResUI.StartService, DateTime.Now.ToString("yyyy/MM/dd HH:mm:ss")));
        await CoreStop(cleanupConfigs: false);
        ...
        {
            await UpdateFunc(true, nodeSummary);
        }
    }
    ...
    public async Task<ProcessService?> LoadCoreConfigSpeedtest(List<ServerTestItem> selecteds)
    {
        ...
        if (result.Success != true)
        {
            FireflyRuntimeArtifactPolicy.CleanupGeneratedConfig(configPath);
            return null;
        }
    ...
    public async Task CoreStop(bool cleanupConfigs = true)
    {
        try
        {
            if (_linuxSudo)
            ...
            if (_processService != null)
            {
                if (!cleanupConfigs)
                {
                    _processService.SuppressConfigCleanup();
                }
            ...
            if (_processPreService != null)
            {
                if (!cleanupConfigs)
                {
                    _processPreService.SuppressConfigCleanup();
                }
        ...
        finally
        {
            if (cleanupConfigs)
            {
                FireflyRuntimeArtifactPolicy.CleanupAllGeneratedConfigs();
            }
        }
    }
    ...
    private async Task<ProcessService?> RunProcess(CoreInfo? coreInfo, string configPath, bool displayLog, bool mayNeedSudo, bool isTunLaunch = false)
    {
        ...
        {
            FireflyRuntimeArtifactPolicy.CleanupGeneratedConfig(Utils.GetBinConfigPath(configPath));
            Logging.SaveLog(_tag, ex);
    ...
    private async Task<ProcessService?> RunProcessNormal(string fileName, CoreInfo? coreInfo, string configPath, bool displayLog)
    {
        ...
        var procService = new ProcessService(
            ...
            cleanupFilePath: Utils.GetBinConfigPath(configPath)
        );
