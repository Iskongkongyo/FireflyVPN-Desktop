...
public sealed class AppManager
{
    ...
    public bool InitApp()
    {
        ...
        SQLiteHelper.Instance.CreateTable<FireflyManagedProfileSecret>();
        SQLiteHelper.Instance.CreateTable<FireflyDeviceCredential>();
...
#pragma warning restore CS0618
        FireflyManagedProfileStorage.ProtectExistingProfilesAsync().GetAwaiter().GetResult();
        return true;
    }
    ...
    public async Task<List<ProfileItem>?> ProfileItems(string subid)
    {
        ...
        {
            return await FireflyManagedProfileStorage.HydrateAsync(await SQLiteHelper.Instance.TableAsync<ProfileItem>().ToListAsync());
        }
        else
        {
            return await FireflyManagedProfileStorage.HydrateAsync(await SQLiteHelper.Instance.TableAsync<ProfileItem>().Where(t => t.Subid == subid).ToListAsync());
        }
    }
    ...
    public async Task<ProfileItem?> GetProfileItem(string indexId)
    {
        ...
        var item = await SQLiteHelper.Instance.TableAsync<ProfileItem>().FirstOrDefaultAsync(it => it.IndexId == indexId);
        return item is null ? null : await FireflyManagedProfileStorage.HydrateAsync(item);
    }
    ...
    public async Task<List<ProfileItem>> GetProfileItemsByIndexIds(IEnumerable<string> indexIds)
    {
        ...
        {
            return await FireflyManagedProfileStorage.HydrateAsync(await SQLiteHelper.Instance.TableAsync<ProfileItem>()
        ...
        for (var size = 0; size < ids.Count; size += Global.SqliteMaxBatchSize)
        {
            ...
            items.AddRange(await FireflyManagedProfileStorage.HydrateAsync(chunkItems));
        }
    ...
    public async Task<ProfileItem?> GetProfileItemViaRemarks(string? remarks)
    {
        ...
        var item = await SQLiteHelper.Instance.TableAsync<ProfileItem>().FirstOrDefaultAsync(it => it.Remarks == remarks);
        return item is null ? null : await FireflyManagedProfileStorage.HydrateAsync(item);
    }
...
#pragma warning restore CS0618
    ...
    public ECoreType GetCoreType(ProfileItem? profileItem, EConfigType eConfigType)
    {
        ...
        return item?.CoreType ?? FireflyBranding.GetDefaultCoreType(eConfigType);
    }
