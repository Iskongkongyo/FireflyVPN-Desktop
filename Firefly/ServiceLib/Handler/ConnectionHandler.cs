...
public static class ConnectionHandler
{
    ...
    public static async Task<IpInfoResult?> GetIPInfo(IWebProxy? webProxy)
    {
        try
        {
            ...
            var country = ipInfo.country ?? ipInfo.country_code ?? ipInfo.countryCode ?? ipInfo.location?.country_code ?? "unknown";
            var countryCode = ipInfo.countryCode ?? ipInfo.country_code ?? ipInfo.location?.country_code;
            var region = ipInfo.region ?? ipInfo.regionCode;

            return new IpInfoResult(country, region, countryCode);
        }
        catch
        {
            return null;
