...
public sealed record DownloadStringResult(string Content);
...
public class DownloadService
{
    ...
    /// <summary>
    /// Tries to download string content and returns the successful response.
    /// </summary>
    ...
    public async Task<DownloadStringResult?> TryDownloadStringWithHeaders(string url, IWebProxy? webProxy, string userAgent)
    {
        ...
        try
        {
            ...
            {
                return new DownloadStringResult(result2);
            }
        }
        catch (Exception ex)
        {
    ...
    private async Task<DownloadStringResult?> DownloadStringWithHeadersAsync(string url, IWebProxy? webProxy, string userAgent, int timeout)
    {
        try
        {
            ...
            return new DownloadStringResult(content);
        }
        catch (Exception ex)
        {
            Logging.SaveLog(_tag, ex);
    ...
    /// </summary>
    internal async Task<WebProxy?> GetWebProxy(bool blProxy)
    {
        if (!blProxy)
        {
            return null;
