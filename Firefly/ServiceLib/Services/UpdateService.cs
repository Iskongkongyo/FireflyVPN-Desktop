...
public class UpdateService(Config config, Func<bool, string, Task> updateFunc)
{
    ...
    private async Task<UpdateResult> ParseDownloadUrl(ECoreType type, UpdateResult result)
    {
        try
        {
            ...
            var coreInfo = CoreInfoManager.Instance.GetCoreInfo(type);
            var coreUrl = await GetUrlFromCore(coreInfo, version) ?? string.Empty;
    ...
    private async Task<string?> GetUrlFromCore(CoreInfo? coreInfo, SemanticVersion version)
    {
        if (Utils.IsWindows())
        {
            if (RuntimeInformation.ProcessArchitecture == Architecture.X86 && coreInfo?.CoreType == ECoreType.mihomo)
            {
                return await GetMihomoWindowsX86Url(coreInfo, version);
            }
    ...
    private async Task<string?> GetMihomoWindowsX86Url(CoreInfo coreInfo, SemanticVersion version)
    {
        var versionText = version.ToVersionString("v");
        var apiUrl = $"{coreInfo.ReleaseApiUrl}/tags/{versionText}";
        var releaseJson = await new DownloadService().TryDownloadString(apiUrl, true, Global.AppName);
        var release = JsonUtils.Deserialize<GitHubRelease>(releaseJson);
        var asset = release?.Assets?
            .Where(a => a.Name?.EndsWith(".zip", StringComparison.OrdinalIgnoreCase) == true)
            .FirstOrDefault(a => Regex.IsMatch(a.Name!, @"^mihomo-windows-386(?:-[A-Za-z0-9]+)*-v\d+(?:\.\d+){1,3}\.zip$"));
